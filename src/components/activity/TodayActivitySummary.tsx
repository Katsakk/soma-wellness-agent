      ...
      <div className="surface-elevated p-5 space-y-4">
        <div className="flex flex-col items-center">
          <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
            ...
            {progress > 0.01 && <path d={describeArc(startAngle, progressAngle)} fill="none" stroke="hsl(var(--metric-activity))" strokeWidth={strokeWidth} strokeLinecap="round" />}
            <g transform={`translate(${center}, ${center})`}>
              <g transform="translate(-12, -50)"><Flame width={24} height={24} stroke="hsl(var(--metric-activity))" fill="hsl(var(--metric-activity))" opacity={0.85} /></g>
              <text textAnchor="middle" y={-4} className="fill-foreground" style={{ fontSize: "36px", fontWeight: 700 }}>{totalCalories}</text>
              <text textAnchor="middle" y={16} className="fill-muted-foreground" style={{ fontSize: "12px" }}>kcal burned</text>
            </g>
        ...
        <div className="flex items-center gap-0 pt-1">
          <StatBar icon={Timer} label="Duration" value={`${totalDuration} min`} colorToken="--metric-activity" progress={Math.min(totalDuration / 60, 1)} />
          <StatBar icon={Dumbbell} label="Workouts" value={`${workoutCount}`} colorToken="--metric-protein" progress={Math.min(workoutCount / 3, 1)} />
        </div>
      </div>
      {activeCategories.length > 0 && (
        <div className="grid grid-cols-3 gap-2">
          {activeCategories.map((cat) => (
            <div key={cat.key} className="surface-elevated p-3 text-center space-y-1">
              <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-semibold">{cat.label}</p>
              <p className="text-sm font-bold" style={{ color: "hsl(var(--metric-activity))" }}>
...
function StatBar({ icon: Icon, label, value, colorToken, progress }: { icon: any; label: string; value: string; colorToken: string; progress: number }) {
  ...
  return (
    <div className="flex-1 px-2">
      ...
      <div className="h-1.5 rounded-full bg-muted overflow-hidden">
        <div className="h-full rounded-full transition-all duration-700" style={{ width: `${Math.max(progress * 100, 2)}%`, backgroundColor: color }} />
      </div>
    </div>
  );
}
