...
export type GmailStatus = "loading" | "connected" | "disconnected";
...
export function useGmailIntegration() {
  ...
  const connect = useCallback(async (returnTo = window.location.pathname) => {
    ...
    if (error || !data?.url) {
      ...
      console.error("gmail-auth error:", error, data);
      return;
    }

    // Try popup first; fall back to full redirect if blocked
    const popup = window.open(data.url, "gmail-oauth", "width=520,height=640,left=200,top=100");
    if (!popup) {
      window.location.href = data.url;
      return;
    }

    // Poll until popup closes, then re-check DB
  ...
  const runSync = useCallback(async (onSuccess?: () => void) => {
    ...
    try {
      ...
      toast.success(
        count > 0
          ? `Imported ${count} workout${count !== 1 ? "s" : ""} from Gmail`
